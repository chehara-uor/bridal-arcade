...
export async function verifyOtpAndRegister(input:{name:string;email:string;phone:string;password:string;otp:string;challenge:string}):Promise<PortalUser>{const response=await axios.post("/api/widget-register",input);if(response.data?.token)setAuthToken(String(response.data.token));return response.data.user as PortalUser;}
